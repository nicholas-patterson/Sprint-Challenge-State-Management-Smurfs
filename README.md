...
- [x] What problem does the context API help solve?
      Answer: The context API is a state management system that stores state at the highest level component. And you can you consumers to get that data without having to prop drill.
- [x] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
      Answer: In Redux, actions and action creators are what we dispatch to the reducers to change the state. Each action is an object with a type and an optional payload. An reducer is what holds the initial state and checks for the actions type coming from the action creator. the store is the holds all the state and is globalize and away from the whole app.

- [x] What is the difference between Application state and Component state? When would be a good time to use one over the other?
      Answer: The difference between application state is redux is application state that holds it globaly and different comps can access it on a need to be basis. Component state is useState useReducer which takes in a initial state.

- [x] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
      Answer: Redux-thunk lets us do asynchronous things with redux. Like use promises and api calls. It changes our action creators because it we need to call dispatch in our creators.

- [x] What is your favorite state management system you've learned and this sprint? Please explain why! My favorite state management system is Redux it makes apps so much cleaner and organized. For smaller projects it would be Context API and useReducer.
